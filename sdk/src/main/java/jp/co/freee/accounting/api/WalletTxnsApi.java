...
public interface WalletTxnsApi {
   ...
   * @param parameters 明細の作成 (optional)
   * @return Observable&lt;WalletTxnsShowResponse&gt;
   */
  @Headers({
    "Content-Type:application/json"
  })
  @POST("api/1/wallet_txns")
  Observable<WalletTxnsShowResponse> createWalletTxn(
    @retrofit2.http.Body CreateWalletTxnParams parameters
  );
  ...
  Observable<WalletTxnsShowResponse> getWalletTxn(
    @retrofit2.http.Path("id") Integer id, @retrofit2.http.Query("company_id") Integer companyId
  );
   ...
   * @param companyId 事業所ID (required)
   * @param walletableType 口座区分 (銀行口座: bank_account, クレジットカード: credit_card, 現金: wallet) (optional)
   * @param walletableId 口座ID (optional)
   ...
   * @return Observable&lt;WalletTxnsIndexResponse&gt;
   */
  @GET("api/1/wallet_txns")
  Observable<WalletTxnsIndexResponse> getWalletTxns(
