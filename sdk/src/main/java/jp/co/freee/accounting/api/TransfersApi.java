...
public interface TransfersApi {
   ...
   * @param parameter 取引（振替）の作成 (optional)
   * @return Observable&lt;TransfersResponse&gt;
   */
  @Headers({
    "Content-Type:application/json"
  })
  @POST("api/1/transfers")
  Observable<TransfersResponse> createTransfer(
    @retrofit2.http.Body TransferParams parameter
  );
  ...
  Observable<TransfersResponse> getTransfer(
    @retrofit2.http.Path("id") Integer id, @retrofit2.http.Query("company_id") Integer companyId
  );
   ...
   * @return Observable&lt;TransfersIndexResponse&gt;
   */
  @GET("api/1/transfers")
  Observable<TransfersIndexResponse> getTransfers(
    @retrofit2.http.Query("company_id") Integer companyId, @retrofit2.http.Query("start_date") String startDate, @retrofit2.http.Query("end_date") String endDate, @retrofit2.http.Query("offset") Integer offset, @retrofit2.http.Query("limit") Integer limit
  );
   ...
   * @param id 取引(振替)ID (required)
   * @param parameter 取引（振替）の更新 (required)
   * @return Observable&lt;TransfersResponse&gt;
   */
  @Headers({
    "Content-Type:application/json"
  ...
  Observable<TransfersResponse> updateTransfer(
    @retrofit2.http.Path("id") Integer id, @retrofit2.http.Body TransferParams parameter
  );
