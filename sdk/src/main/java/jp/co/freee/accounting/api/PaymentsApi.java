...
public interface PaymentsApi {
   ...
   * @param parameters 取引（収入／支出）の支払行作成 (required)
  ...
  Observable<DealsResponse> createDealPayment(
    @retrofit2.http.Path("id") Integer id, @retrofit2.http.Body DealPaymentParams parameters
  );
   ...
   * @param id 取引ID (required)
   * @param paymentId 決済ID (required)
   * @param parameters 取引（収入／支出）の支払行更新 (required)
   * @return Observable&lt;DealsResponse&gt;
   */
  @Headers({
    "Content-Type:application/json"
  ...
  Observable<DealsResponse> updateDealPayment(
    @retrofit2.http.Path("id") Integer id, @retrofit2.http.Path("payment_id") Integer paymentId, @retrofit2.http.Body DealPaymentParams parameters
  );
